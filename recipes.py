...
def list_recipes():
    sql = "SELECT id, title FROM recipes WHERE visible=1 ORDER BY id DESC"
...
def get_recipe_tags(recipe_id):
    sql = "SELECT T.tag FROM tags T, recipetags R WHERE R.recipe_id=:recipe_id AND T.id=R.tag_id AND R.visible=1"
    tags = db.session.execute(sql, {"recipe_id":recipe_id}).fetchall()
    return tags

def get_recipe_ingredients(recipe_id):
    sql = "SELECT ingredient FROM ingredients WHERE recipe_id=:recipe_id AND visible=1"
...
def add_recipe(title, description, instruction, ingredients, tags):
    check_ok, msg = check_recipe_inputs(0, title, description, instruction, ingredients)
    if not check_ok:
...
def modify_recipe(recipe_id, title, description, instruction, ingredients, tags):
    check_ok, msg = check_recipe_inputs(recipe_id, title, description, instruction, ingredients)
    if not check_ok:
        return False, msg
...
def title_taken(title, recipe_id):
    sql = "SELECT COUNT(*) FROM recipes WHERE title=:title AND visible=1 AND id<>:recipe_id"
    result = db.session.execute(sql, {"title":title, "recipe_id":recipe_id}).fetchone()[0]
    return result

def is_visible(recipe_id):
    sql = "SELECT visible FROM recipes WHERE id=:recipe_id"
